...
void cub_exclusiveSum(IdType* arrays, const IdType array_length) {
  ...
  cub::DeviceScan::ExclusiveSum(
      d_temp_storage, temp_storage_bytes, arrays, arrays, array_length);
  d_temp_storage =
...
void cub_sortPairs(
    KeyType* d_keys_in, KeyType* d_keys_out, ValueType* d_values_in,
    ValueType* d_values_out, int32_t num_items, int num_bits) {
  if (num_bits == 0) {
...
template <typename KeyType, typename ValueType>
void cub_segmentedSortPairsDescending(
    KeyType* d_keys_in, KeyType* d_keys_out, ValueType* d_values_in,
    ValueType* d_values_out, int64_t* d_offsets, int32_t num_items,
...
void cub_segmentedSum(
    DType* d_in, DType* d_out, IdType* d_offsets, int64_t num_segments) {
  void* d_temp_storage = NULL;
  size_t temp_storage_bytes = 0;
  cub::DeviceSegmentedReduce::Sum(
      d_temp_storage, temp_storage_bytes, d_in, d_out, num_segments, d_offsets,
      d_offsets + 1);
  // Allocate temporary storage
  d_temp_storage =
      c10::cuda::CUDACachingAllocator::raw_alloc(temp_storage_bytes);
...
template <typename IdType>
void cub_consecutiveUnique(
    IdType* d_in, IdType* d_out, IdType* d_num_selected_out,
    int64_t num_items) {
  void* d_temp_storage = NULL;
  size_t temp_storage_bytes = 0;
  cub::DeviceSelect::Unique(
      d_temp_storage, temp_storage_bytes, d_in, d_out, d_num_selected_out,
      num_items);
  // Allocate temporary storage
  d_temp_storage =
      c10::cuda::CUDACachingAllocator::raw_alloc(temp_storage_bytes);
...
template <typename IdType>
void cub_segmentSort(
    IdType* d_keys_in, IdType* d_keys_out, IdType* d_offsets, int64_t num_items,
