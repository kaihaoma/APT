...
def draw_multimachines_mainexp(path, output_prefix="./outputs/figures/mult-machine/"):
    ...
    for lid, element in enumerate(elements):
        ...
        x_val = nl
        y_val = epoch_time
        print(f"[Note] graph:{graph}\t sys:{sys}\t model:{model}\t gpu_cache_mem:{gpu_cache_mem}\t nl:{nl}\t xval:{x_val}\t yval:{y_val}")
    ...
    for file in sorted(os.listdir(path_prefix)):
        ...
        if len(plot_x_list) == 5:
            draw_utils.plot_line(
                ...
                ylabels="Accuracy",
                save_path=f"./outputs/figures/accuracy/_{model}.png",
            )
            plot_x_list = []
            plot_y_list = []
            labels = []
...
if __name__ == "__main__":
    ...
    # multi-machines
    multi_machine_path_prefix = "./outputs/speed/"
    ...
    for path in path_list:
        draw_multimachines_mainexp(path=path, output_prefix="./outputs/figures/multi-machine/")

    # single machine
    single_machine_path_list = []
    single_machine_path_list.append("./outputs/speed/single_machine/NP_friendster_igb.csv")
    draw_mainexp(path_list=single_machine_path_list, fixed="nl", output_prefix="./outputs/figures/single-machine")
