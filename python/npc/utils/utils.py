    ...
    if args.nproc_per_node != -1:
        ...
        if args.num_localnode_feats_in_workers == -1:
            ...
            localnode_feats = (
                ...
                else torch.rand((total_nodes, input_dim), dtype=torch.float32)
            )
            print(
                f"[Note]Localnode feats: ALL :{args.system}\t localnode_feats shape:{localnode_feats.shape}"
            )
        elif args.system == "MP":
            args.num_localnode_feats = total_nodes
            localnode_feats_idx = torch.arange(total_nodes)
            ...
            localnode_feats = (
                global_node_feats[
                    :, args.cumsum_feaet_dim[st] : args.cumsum_feat_dim[en]
                ]
                if global_node_feats is not None
                else torch.rand(
                    (args.num_localnode_feats, mp_input_dim), dtype=torch.float32
                )
            )
            print(
                f"[Note]MP Localnode feats: ALL :{args.system}\t localnode_feats shape:{localnode_feats.shape}"
            )
        else:
            ...
            num_localnode_feats = int(
                args.num_localnode_feats_in_workers
                * 1024
                * 1024
                * 1024
                / (4 * input_dim)
            ...
            if args.num_localnode_feats <= min_req:
                ...
                print(
                    f"[Note]load dryrun for sys {args.system} from {args.dryrun_file_path}"
                )
                if args.nproc_per_node != -1 and args.hybrid:
                    # hybrid parallelism
                    local_freq_lists = [
                        torch.load(f"{args.dryrun_file_path}/rk#{r}_epo10.pt")[1]
                        for r in range(st, en)
                    ]
                else:
                    local_freq_lists = [
                        torch.load(f"{args.dryrun_file_path}/rk#{r}_epo100.pt")[1]
                        for r in range(st, en)
                    ]
            ...
            localnode_feats = (
                ...
                if global_node_feats is not None
                    ...
                    (localnode_feats_idx.numel(), input_dim), dtype=torch.float32
                )
            )
...
def load_graph(args):
    ...
    torch_graph_path = graph_path.split(".")[0] + "_torch.bin"
    if os.path.isfile(torch_graph_path):
        print(f"[Note]Load graph from {torch_graph_path}")
        graph = torch.load(torch_graph_path)
    else:
        print(f"[Note]Load graph from {graph_path}")
        dataset_tuple = dgl.load_graphs(graph_path)
        graph = dataset_tuple[0][0]
        torch.save(graph, graph_path.split(".")[0] + "_torch.bin")
    return graph
...
def init_args(args=None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--logs_dir", type=str, default="./outputs/time.csv", help="log file dir"
    )
    parser.add_argument(
        "--costmodel_log",
        type=str,
        default="./outputs/costmodel.txt",
        help="costmodel log file",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--system", type=str, default="NP", choices=["DP", "NP", "SP", "MP"]
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="/efs/khma/Projects/NPC/sampling_all/ap_simulation",
    )
    parser.add_argument(
        "--dp_freqs_path",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=4,
        help="number of attention heads, only for GAT",
    )
