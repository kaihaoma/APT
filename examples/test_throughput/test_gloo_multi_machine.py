...
def run_unidirection(rank, world_size, backend, data_size):
    def run_once():
        if rank == 0:
            tensor = torch.randn(data_size, dtype=torch.float32)
...
def run_bidirection(rank, world_size, backend, data_size):
    def run_once():
        input = torch.randn(data_size, dtype=torch.float32)
        output = torch.randn(data_size, dtype=torch.float32)
...
if __name__ == "__main__":
    ...
    for data_size in [1000000, 10000000, 100000000, 500000000, 1000000000]:
        ...
        for rank in range(num_workers_per_node):
            p = mp.Process(target=init_process, args=(rank + args.node * num_workers_per_node, world_size, func, "gloo", data_size))
            p.start()
            processes.append(p)
