...
def run(rank, args, shared_queue, shared_tensor_list):
    ...
    # define model
    if args.system == "NPC":
        training_model = NPCSAGE(in_feats=args.input_dim,
        ...
        training_model = DGLSAGE(in_size=args.input_dim, hid_size=args.num_hidden,
                                 out_size=args.num_classes, num_layers=num_layers).to(device)
    ...
    if training_mode == 'sampling':
        ...
        for epoch in range(num_epochs):
            for step, sample_result in enumerate(dataloader):
                ...
                record.extend([num_cached, num_sample_nodes,
                              num_cached / num_sample_nodes])
        ...
        tensor_list = [torch.zeros(num_data_types*3, device=device)
        ...
        if rank == 0:
            ...
            if True:
                with open(args.logs_dir, 'a') as f:
                    ...
                    log_info = [args.tag, args.system, args.dataset, args.model, args.machine, args.batch_size,
                                args.fan_out.replace(',', ' '), args.num_hidden, args.feat_cache_ratio,]
        ...
        if rank == 0:
            ...
            avg_time_epoch_training = round(
                total_time[2] * 1000. / num_record_epochs, 4)
            print(f"[Note]Write to logs file {args.logs_dir}")
            with open(args.logs_dir, 'a') as f:
                writer = csv.writer(f, lineterminator='\n')
                ...
                log_info = [args.tag, args.system, args.dataset, args.model, args.machine, args.batch_size,
                            args.fan_out.replace(',', ' '), args.num_hidden, args.feat_cache_ratio, args.graph_cache_ratio, num_record_epochs, num_batches_per_epoch, avg_time_epoch_sampling, avg_time_epoch_loading, avg_time_epoch_training]
                writer.writerow(log_info)
...
if __name__ == "__main__":
    ...
    if len(list(graph.ndata.keys())) > 1:
        ...
        global_node_feats = torch.rand(
            (num_nodes, input_dim), dtype=torch.float32)
    ...
    shared_tensor_list = [global_node_feats,
                          global_labels, global_train_mask, indptr, indices,]
    mp.spawn(run,
